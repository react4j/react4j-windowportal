...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/react4j/react4j-windowportal/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']})
[Full Changelog](https://github.com/react4j/react4j-windowportal/compare/v#{ENV['PREVIOUS_PRODUCT_VERSION']}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('react4j/react4j-windowportal', tag, :name => tag, :body => changes, :draft => false, :prerelease => true)

      candidates = client.list_milestones('react4j/react4j-windowportal').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('react4j/react4j-windowportal', milestone[:number], :state => 'closed')
        end
      end
    end
  end
